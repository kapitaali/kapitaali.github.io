;(_ => {
 ...
 <style>
  .ngn_lb{position:fixed;top:0;left:0;right:0;font-size:25px;background-color:#eee;color:#000;cursor:default;z-index:2147483647;
  ...
  .ngn_x:hover{color:#f00}
    ...
    document.body.appendChild(rootDivElement)
    let t, ts = [], lb = rootDivElement.firstChild, bqm = 0 //t:textarea or input, lb:language bar, bqm:backquote mode
    ...
    let fk = x => {
        ...
        if (bqm) {
            ...
            if (x.which > 31) {
                bqm = 0;
                document.body.classList.remove('ngn_bq')
            }
            if (c) {
        ...
        if (!x.ctrlKey && !x.shiftKey && !x.altKey && !x.metaKey) {
            if ("`½²^º§ùµ°".indexOf(x.key) > -1) {
                bqm = 1;
                document.body.classList.add('ngn_bq');
                pd(x); // ` or other trigger symbol pressed, wait for next key
            } else if (x.key == "Tab") {
    ...
    let upd = _ => {
        document.body.style.marginTop = lb.clientHeight + 'px'
    }
    upd();
    ev(window, 'resize', upd)
    ev(document, 'focus', ff, !0);
